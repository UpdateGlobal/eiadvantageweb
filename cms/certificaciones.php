...
if ($eliminar == "true") {
  $sqlEliminar = "SELECT cod_certifica FROM certificaciones ORDER BY orden";
  $sqlResultado = mysqli_query($enlaces,$sqlEliminar);
  $x = 0;
  while($filaElim = mysqli_fetch_array($sqlResultado)){
    $id_certifica = $filaElim["cod_certifica"];
    if ($_REQUEST["chk" . $id_certifica] == "on") {
      $x++;
      if ($x == 1) {
        $sql = "DELETE FROM certificaciones WHERE cod_certifica=$id_certifica";
      } else {
        $sql = $sql . " OR cod_certifica=$id_certifica";
      }
    }
  }
        ...
        <div class="row">
          <div class="col-md-12">
            <div class="card card-bordered">
              <h4 class="card-title"><strong>Lista Certificaciones</strong></h4>
              <div class="card-body">
                <a class="btn btn-info" href="<?php if($xVisitante=="0"){ ?>certificaciones-nuevo.php<?php }else{ ?>javascript:visitante();<?php } ?>"><i class="fa fa-plus"></i> A&ntilde;adir nuevo</a>
                <hr>
                    ...
                    <tbody>
                      <?php
                        ...
                        while($filaCer = mysqli_fetch_array($resultadoCertifica)){
                          $xCodigo    = $filaCer['cod_certifica'];
                          $xTitulo    = $filaCer['titulo'];
                          $xFecha     = $filaCer['fecha'];
                          $xOrden     = $filaCer['orden'];
                          $xEstado    = $filaCer['estado'];
                      ?>
                      <tr>
                        ...
                        <td>
                          <a class="boton-eliminar <?php if($xVisitante=="1"){ ?>boton-eliminar-bloqueado<?php } ?>" href="<?php if($xVisitante=="0"){ ?>certificaciones-delete.php?cod_certifica=<?php echo $xCodigo; ?><?php }else{ ?>javascript:visitante();<?php } ?>">
                            <i class="fa fa-trash" aria-hidden="true"></i>
                          </a>
                        </td>
                        <td><a class="boton-editar" href="certificaciones-edit.php?cod_certifica=<?php echo $xCodigo; ?>"><i class="fa fa-pencil-square" aria-hidden="true"></i></a></td>
                        <td>
                          <?php if($xVisitante=="0"){ ?>
                          <div class="hidden">
