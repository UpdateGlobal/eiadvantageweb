...
if ($eliminar == "true") {
  $sqlEliminar = "SELECT cod_menu FROM contenidos_categorias ORDER BY orden";
  $sqlResultado = mysqli_query($enlaces,$sqlEliminar);
  $x = 0;
  while($filaElim = mysqli_fetch_array($sqlResultado)){
    $id_menu = $filaElim["cod_menu"];
    if ($_REQUEST["chk" . $id_menu] == "on") {
      $x++;
      if ($x == 1) {
          $sql = "DELETE FROM contenidos_categorias WHERE cod_menu=$id_menu";
        }else{
          $sql = $sql . " OR cod_menu=$id_menu";
        }
    }
  }
  ...
  <head>
    ...
    <style>
      @media only screen and (max-width: 760px), (min-device-width: 768px) and (max-device-width: 1024px) {
        ...
        td:nth-of-type(2):before { content: "Dropdown(?)"; }
                  ...
                  <table class="table">
                    <thead>
                      <tr>
                        ...
                        <th width="10%" scope="col">Dropdown(?)</th>
                    ...
                    <tbody>
                      <?php
                        $consultarCategoria = "SELECT * FROM contenidos_categorias ORDER BY orden";
                        $resultadoCategoria = mysqli_query($enlaces,$consultarCategoria) or die('Consulta fallida: ' . mysqli_error($enlaces));
                        while($filaCat = mysqli_fetch_array($resultadoCategoria)){
                          $xCodigo    = $filaCat['cod_menu'];
                          $xCategoria = $filaCat['categoria'];
                          $xDropdown  = $filaCat['dropdown'];
                      ...
                      <tr>
                        ...
                        <td>
                          <strong><?php if($xDropdown=="1"){ echo "[Si]"; }else{ echo "[No]";} ?></strong>
                        </td>
                        ...
                        <?php }?></td>
                        <td><?php if($xCodigo!="0"){?>
                          <?php if($xVisitante=="0"){ ?><a class="boton-eliminar" href="contenidos-categorias-delete.php?cod_menu=<?php echo $xCodigo; ?>"><i class="fa fa-trash"></i></a><?php }else{ ?><a class="boton-eliminar boton-eliminar-bloqueado" href="javascript:visitante();"><i class="fa fa-trash"></i></a><?php } ?><?php }?>
                        </td>
                        <td><?php if($xCodigo!="0"){?><a class="boton-editar" href="contenidos-categorias-edit.php?cod_menu=<?php echo $xCodigo; ?>"><i class="fa fa-pencil-square"></i></a><?php }?></td>
