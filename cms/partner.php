...
if ($eliminar == "true") {
  $sqlEliminar = "SELECT cod_partner FROM partner ORDER BY fecha_ingreso";
  $sqlResultado = mysqli_query($enlaces,$sqlEliminar) or die('Consulta fallida: ' . mysqli_error($enlaces));
  $x = 0;
  while($filaElim = mysqli_fetch_array($sqlResultado)){
    $id_partner = $filaElim["cod_partner"];
    if ($_REQUEST["chk" . $id_partner] == "on"){
      $x++;
      if ($x == 1) {
          $sql = "DELETE FROM partner WHERE cod_partner=$id_partner";
        } else {
          $sql = $sql . " OR cod_partner=$id_partner";
        }
    }
  }
        ...
        <div class="row">
          <div class="col-md-12">
            <div class="card card-bordered">
              <h4 class="card-title"><strong>Lista de Partners</strong></h4>
              <div class="card-body">
                <form name="fcms" method="post" action="">
                  <table class="table" data-provide="datatables">
                    ...
                    <tbody>
                      <?php
                        $consultarContacto = "SELECT * FROM partner ORDER BY fecha_ingreso";
                      ...
                      <tr>
                        ...
                        <td><a class="boton-editar" href="partner-edit.php?cod_partner=<?php echo $xCodigo; ?>"><i class="fa fa-search" aria-hidden="true"></i></a></td>
                        <td>
                          <?php if($xVisitante=="0"){ ?>
                          <div class="hidden">
